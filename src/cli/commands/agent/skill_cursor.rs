...
impl SkillCursor {
    ...
    /// because the cursor went inactive. The 2026-08-28 pkg-mcp e2e is the
    ...
    /// `path:` where Pack needs `localPath:`; then the umbrella step guarding
    ...
    /// longer identifier or a file path (`chart/templates/pkg-package.yaml`)
    /// is not a handoff.
    ...
    /// (`chart/templates/pkg-package.yaml`), and it refuses an ancestor
    ...
    /// the first: the real `pkg-package-build` final step reads "Continue with
    /// the pkg-package-integrate skill. … the Integration Phase invokes
    /// `validate-package` through the `pkg-package-validate` skill."
    ...
    /// Integration Phase's job, invoke pkg-package-integrate") is a passing
...
/// followed by a file extension (`.yaml` in `pkg-package.yaml`) — a
...
pub fn active_step_block(config: &Config) -> Option<String> {
    ...
    // and a model complies: the live pkg-mcp e2e marked the build skill's
...
mod tests {
    ...
    fn a_frames_last_step_is_never_auto_advanced() {
        ...
        // `DeployReviewWorkspace`, the integrate skill's terminal step, with
    ...
    fn the_20260828_e2e_chain_leaves_a_cursor_for_the_retry_to_resume() {
        ...
        c.descend(
            ...
            steps(&["ImplementConfigChart", "DeployReviewWorkspace"]),
        );
        c.mark_abandoned(); // round cap retires ImplementConfigChart mid-write
        assert!(
            !c.may_auto_advance(),
            "DeployReviewWorkspace is terminal — no valve may retire it"
        );
    ...
    fn handoff_requires_token_boundaries_not_raw_containment() {
        ...
        // mention, not a handoff — `chart/templates/pkg-package.yaml` must
        // not descend into pkg-package.
        let installed = vec!["pkg-package".to_string(), "pkg".to_string()];
        let c = cur(&["Create chart/templates/pkg-package.yaml from the template"]);
        assert_eq!(c.handoff_target(&installed), None);

        let c = cur(&["Invoke pkg-package skill"]);
        assert_eq!(c.handoff_target(&installed).as_deref(), Some("pkg-package"));
        // longest name still wins over its own substring
        let c = cur(&["Invoke the pkg-package skill, part of pkg"]);
        assert_eq!(c.handoff_target(&installed).as_deref(), Some("pkg-package"));
        // sentence-ending period is a boundary, an extension is not
        let c = cur(&["Finish with pkg-package."]);
        assert_eq!(c.handoff_target(&installed).as_deref(), Some("pkg-package"));
        let c = cur(&["Edit my-pkg-package now"]);
        assert_eq!(c.handoff_target(&installed), None);
    }

    #[test]
    fn contains_token_boundary_cases() {
        assert!(contains_token("invoke pkg-package skill", "pkg-package"));
        assert!(contains_token("pkg-package", "pkg-package"));
        assert!(contains_token("(pkg-package)", "pkg-package"));
        assert!(contains_token("run pkg-package.", "pkg-package"));
        assert!(!contains_token("chart/pkg-package.yaml", "pkg-package"));
        assert!(!contains_token("my-pkg-package", "pkg-package"));
        assert!(!contains_token("pkg-packages", "pkg-package"));
        assert!(!contains_token("pkg-package_v2", "pkg-package"));
        // rejected first occurrence must not mask a valid later one
        assert!(contains_token(
            "pkg-package.yaml uses pkg-package",
            "pkg-package"
        ));
    }
    ...
    fn body_handoff_falls_back_to_the_distilled_body() {
        ...
        c.cache("Continue with the pkg-package-integrate skill.".into());
        assert_eq!(
    ...
    /// The real `pkg-package-build` SKILL.md tail, verbatim.
    fn build_skill_dir() -> tempfile::TempDir {
        let tmp = tempfile::tempdir().unwrap();
        std::fs::write(
            tmp.path().join("SKILL.md"),
            "# pkg-package-build\n\n## Step CreatePKGBundle: Assemble the bundle\n\
             Write `bundle/pkg-bundle.yaml` referencing chart/templates/pkg-package.yaml.\n\n\
             ## Step EnterIntegrationPhase: Next phase\n\
             Continue with the pkg-package-integrate skill. Do not treat `pkg pack dev lint` \
             as the Validation Phase; the Integration Phase invokes `validate-package` through \
             the `pkg-package-validate` skill.\n",
        )
        .unwrap();
        tmp
    }
    ...
    fn build_cursor(dir: &Path) -> SkillCursor {
        ...
        c.push_skill(
            ...
            vec![
                SkillStep {
                    name: "CreatePKGBundle".into(),
                    anchor: "## Step CreatePKGBundle: Assemble the bundle".into(),
                },
                SkillStep {
    ...
    fn body_handoff_ignores_skills_named_in_sibling_docs() {
        ...
        std::fs::write(
            ...
            "See also the pkg-package-publish skill for registry pushes.\n",
        )
        .unwrap();
        let mut c = build_cursor(tmp.path());
        c.mark_done();
        let installed = vec!["pkg-package-publish".to_string()];
        assert_eq!(
            first_candidate(&c, &installed),
            None,
    ...
    fn handoff_prefers_longest_installed_name() {
        let mut c = cur(&["Invoke pkg-package-build skill"]);
        // both substrings present; the longer, more specific name wins,
        // and an already-loaded skill is excluded.
        let got = c.handoff_target(&["pkg-package".into(), "pkg-package-build".into()]);
        assert_eq!(got.as_deref(), Some("pkg-package-build"));
        c.push_skill("pkg-package-build", Path::new("/x"), steps(&["S"]));
        // now loaded -> no further handoff to it from that name
        assert_eq!(c.handoff_target(&["pkg-package-build".into()]), None);
    }
    ...
    fn active_block_is_absent_until_the_step_is_distilled() {
        ...
        c.cache("call scaffold-feature with targetDir".into());
        save(&config, &c);
        let block = active_step_block(&config).unwrap();
        assert!(block.contains("[SKILL STEP]"));
        assert!(block.contains("call scaffold-feature"));

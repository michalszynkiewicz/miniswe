...
/// Fast-mode models also loop on `replace_range` (same range + same bytes)
/// and on `revert` (same rev), so fast mode keeps the guard but points the
/// model at the revision-table tools (`show_rev`, a different `rev`) rather

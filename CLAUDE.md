...
* always restart the llama-server between bench runs (a long-running instance introduces uptime as an uncontrolled variable — GPU memory fragmentation, thermal throttling, etc. — that can confound comparisons between arms run at different points in its uptime)

#!/usr/bin/env bash
# run-opencode-bench.sh — opencode bench rooted at the repo with no file hints.
#
# Mirrors run-aider-bench.sh: opencode is invoked in its own fresh Docker
# container, on the SAME pinned baseline SHA, SAME task, and SAME 6-check
# validation suite as run-benchmark-docker.sh (miniswe) and run-aider-bench.sh
# (aider) — the closest apples-to-apples comparison across all three agents.
#
# opencode needs a config file (not just env vars, unlike aider) to talk to
# a local OpenAI-compatible endpoint: ~/.config/opencode/opencode.json for
# the provider/model definition, and ~/.local/share/opencode/auth.json for
# a dummy API key (opencode requires one to exist even though llama-server
# doesn't check it). Both are generated here (model ID discovered from
# /v1/models, same mechanism run-compaction-bench.sh already uses for its
# MODEL_TAG) and mounted read-only into the container.
#
# Usage:
#   ./scripts/run-opencode-bench.sh [--timeout 1800] [--max-attempts 3]
...
# Raw model ID as exposed by /v1/models (used verbatim in opencode.json —
# opencode's docs say "the model key must exactly match the model ID exposed
# by the backend"). Distinct from MODEL_TAG below, which is a sanitized
# short label used only for the results directory name.
MODEL_ID="$(
...
RESULTS_DIR="${REPO_DIR}/benchmark_results/opencode_$(date +%Y%m%d_%H%M%S)_${MODEL_TAG}"
...
echo "=== Docker-isolated opencode benchmark ==="
...
# ── opencode config (provider/model + dummy auth) ───────────────────────
...
run_variant() {
    ...
    local container_name="miniswe-opencode-${name}-$$"
    mkdir -p "${variant_dir}"
    echo "--- ${name} ---"
...
while [ "$ATTEMPT" -lt "$MAX_ATTEMPTS" ]; do
    ...
    # opencode in one-shot mode, rooted at /work via --dir. --auto
    # auto-approves permissions (mirrors miniswe's --yes / aider's
    # --yes-always) so the run doesn't hang on a confirmation prompt.
    timeout "${REMAINING}" opencode run "${CURRENT_TASK}" \
        --model "${MODEL_REF}" \
        --dir /work \
        --auto \
        --print-logs \
    ...
    # === Validate (same 6 checks as run-benchmark-docker.sh / run-aider-bench.sh) ===
    PASS=0
    TOTAL=0
    ERRORS=""

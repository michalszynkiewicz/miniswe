#!/usr/bin/env bash
# run-benchmark-docker-fast.sh — Same benchmark harness as
# run-benchmark-docker.sh, but runs miniswe with `edit_mode = "fast"`.
#
# Fast mode replaces the inner-model edit_file planner with primitive
# range-based edits (replace_range / insert_at / revert) and per-edit
# AST + LSP feedback. See docs/fast-mode-design.md.
#
# Usage:
#   ./scripts/run-benchmark-docker-fast.sh [--timeout 1800] [--max-rounds 50] \
...
#                --port 8464 -c 60000
#   # then: ./scripts/run-benchmark-docker-fast.sh --model gemma-4-26B-A4B-it
...
# self-labeling. Falls back to "unknown" if the server isn't reachable.
...
RESULTS_DIR="${REPO_DIR}/benchmark_results/docker_fast_$(date +%Y%m%d_%H%M%S)_${MODEL_TAG}"
...
echo "Model:    ${MODEL}"
echo "Mode:     fast (replace_range / insert_at / revert + AST+LSP feedback)"
...
plan = $(_dis plan)
scratchpad = $(_dis scratchpad)
# Fast-mode toggle: swaps edit_file for the replace_range / insert_at /
# revert primitive surface with per-edit AST + LSP feedback. See
# docs/fast-mode-design.md. Silently ignored by builds that don't yet
# implement fast mode.
edit_mode = "fast"
...
SCRIPT
    ...
    # See run-benchmark-docker.sh for rationale on MINISWE_LLM_DUMP_DIR.
    docker run --rm \
        --network=host \
...
# Run baseline with all tools enabled, but in fast edit mode
echo "═══ Baseline (all tools, fast edit mode) ═══"
echo ""

run_variant "00_baseline_fast" ""

# Summary
echo ""
echo "================================================================="
echo "  RESULTS (fast mode)"
echo "================================================================="
printf "%-24s %8s %4s %8s %s\n" "Variant" "Rounds" "Att" "Time" "Result"
echo "-----------------------------------------------------------------"

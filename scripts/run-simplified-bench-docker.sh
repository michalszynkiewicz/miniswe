#!/usr/bin/env bash
# run-simplified-bench-docker.sh — Same as run-benchmark-docker.sh but the
# task prompt names the canonical file set up front, matching the head
# start that run-simplified-aider-bench.sh gives aider via --file flags.
#
# Goal: an apples-to-apples comparison between miniswe and aider where
# neither agent pays the discovery cost. Both get the same scope hint;
# both have to do the actual editing.
#
# Each variant runs in its own container — completely fresh filesystem,
# no shared target/, no stale binaries.
#
# Usage:
#   ./scripts/run-simplified-bench-docker.sh [--timeout 1800] [--max-rounds 50] \
...
RESULTS_DIR="${REPO_DIR}/benchmark_results/docker_scoped_$(date +%Y%m%d_%H%M%S)_${MODEL_TAG}"
...
It is enough to change these files:
- src/main.rs
- src/cli/mod.rs

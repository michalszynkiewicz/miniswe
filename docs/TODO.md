...
→ false) instead of running the YAML grammar. Plain `values.yaml`, `zarf.yaml`,
`Chart.yaml`, `uds-bundle.yaml` keep the syntax + duplicate-key check. Unit
...
typescript-language-server, pyright, gopls, clangd, jdtls — a UDS package repo
...
yaml-language-server (schema validation for zarf.yaml etc.) is a later opt-in.
...
- `LspServer::HelmLs`: detect AFTER all existing language checks (single
  ...
  `charts/*/Chart.yaml`, the UDS `**/chart/Chart.yaml` layout).
